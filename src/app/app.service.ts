...
import { getAllBrands, getSlider } from './shared/api/api';
...
export class AppService {
    ...
    public url = "assets/data/";
        // public uppdatedURL="https://dev-ecommerce-api.sdex.online/api";
    public uppdatedURL="http:///209.145.58.10:8080";

    constructor(public http:HttpClient, public snackBar: MatSnackBar) { 
          // ** when not exist cart list in localstorage
          if (!localStorage.getItem('cartList')) {
            localStorage.setItem('cartList', JSON.stringify(this.Data.cartList))
        } 

        // ** when not exist compare list in localstorage
        if (!localStorage.getItem('compareProducts')) {
        localStorage.setItem('compareProducts', JSON.stringify(this.Data.compareList))
    } 
    }
    
    public getCategories(): Observable<Category[]>{
        return this.http.get<Category[]>(this.uppdatedURL + '/api/PoductCategory/GetCategories?Offset=0&Limit=60');
    ...
    public getSliders(): Observable<any[]>{
        ...
        return this.http.get<any[]>(`${getSlider}` + params);
    }

    public addToCompare(product:Product){
        let message, status;
        if(this.Data.compareList.filter(item=>item.id == product.id)[0]){
            message = 'This product is already added to comparison list.'; 
            status = 'error';     
        }
        else{
            this.Data.compareList.push(product);
            localStorage.setItem('compareProducts', JSON.stringify(this.Data.compareList));
            this.compareTotal = JSON.parse(localStorage.getItem('compareProducts'));
            message = 'The product ' + product.productTranslations[0].title + ' has been added to comparison list.'; 
            status = 'success';  
        }
    ...
    public addToCart(product:Product){
        ...
        let x = JSON.parse(localStorage.getItem('cartList'));
        ...
        localStorage.setItem('cartList', JSON.stringify(this.Data.cartList)); // set item in localStorage
        
        this.Data.cartList.forEach (product =>{ 
            
            this.Data.totalPrice = this.Data.totalPrice + (product.discount ? product.cartCount * product.price - product.discount : product.cartCount * product.price);
            this.Data.totalCartCount = this.Data.totalCartCount + product.cartCount;
            this.productId = product.id;
            this.productQuantity = product.cartCount;
        ...
        message = 'The product ' + product.productTranslations[0].title + ' has been added to cart.'; 
        status = 'success';          
        this.snackBar.open(message, '×', { panelClass: [status], verticalPosition: 'top', duration: 3000 });
    }
    ...
    public getDeliveryMethods(){
        return [
            { value: 'free', name: 'Cash On Delivery' },
